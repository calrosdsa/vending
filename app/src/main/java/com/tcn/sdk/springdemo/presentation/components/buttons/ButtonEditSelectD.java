...
public class ButtonEditSelectD extends RelativeLayout {
    ...
    /*
    //输入类型为没有指定明确的类型的特殊内容类型  The input type is a special content type without specifying an explicit type
    editText.setInputType(InputType.TYPE_NULL);

    //输入类型为普通文本	Input type is plain text

    editText.setInputType(InputType.TYPE_CLASS_TEXT);

    //输入类型为数字文本	Input type is numeric text
    editText.setInputType(InputType.TYPE_CLASS_NUMBER);

    //输入类型为电话号码  Input type is phone number
    editText.setInputType(InputType.TYPE_CLASS_PHONE);

    //输入类型为日期和时间  Input type is date and time
